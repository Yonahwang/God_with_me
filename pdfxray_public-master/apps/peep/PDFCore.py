...
class PDFDictionary (PDFObject):
    def __init__(self, rawContent = '', elements = {}, rawNames = {}) :
        ...
        self.references = []
        self.suspiciousEvents = [] #9bplus
        self.suspiciousActions = [] #9bplus
        self.suspiciousElements = [] #9bplus
        self.vulns = [] #9bplus
        ret = self.update()
        if ret[0] == -1:
            if isForceMode:
    ...
    def setSuspiciousEvents(self,event): #9bplus
        self.suspiciousEvents.append(event) #9bplus
        
    def setSuspiciousActions(self,action): #9bplus
        self.suspiciousActions.append(action) #9bplus
        
    def setSuspiciousElements(self,element): #9bplus
        self.suspiciousElements.append(element) #9bplus
        
    def setVulns(self,vuln): #9bplus
        self.vulns.append(vuln) #9bplus
        ...
        for event in monitorizedEvents:
            if value.find(event) != -1:
                if self.suspiciousEvents.has_key(event):
                    ...
                    pdfObject.setSuspiciousEvents(event) #9bplus
        for action in monitorizedActions:
            if value.find(action) != -1:
                if self.suspiciousActions.has_key(action):
                    ...
                    pdfObject.setSuspiciousActions(action) #9bplus
        for element in monitorizedElements:
            if value.find(element) != -1:
                if self.suspiciousElements.has_key(element):
                    ...
                    pdfObject.setSuspiciousElements(element) #9bplus
